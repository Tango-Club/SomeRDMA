...
bool LocalEngine::write(const std::string key, const std::string value) {
   ...
   * RDMA write in the background */
    ...
    int ret = m_rdma_conn_->remote_write((void*)value.c_str(), value.size(), remote_addr, rkey);
    if (ret) return false;
...
bool LocalEngine::read(const std::string key, std::string& value) {
    ...
    if (m_rdma_conn_->remote_read((void*)value.c_str(), inter_val.size, inter_val.remote_addr,
                                  inter_val.rkey))
        return false;
