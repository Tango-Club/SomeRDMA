...
int RemoteEngine::remote_write(WorkerInfo* work_info, uint64_t local_addr, uint32_t lkey,
...
void RemoteEngine::worker(WorkerInfo* work_info, uint32_t num) {
    ...
    while (true) {
        ...
        if (request->type == MSG_REGISTER) {
            ...
            /* write response */
            remote_write(work_info, (uint64_t)cmd_resp, resp_mr->lkey, sizeof(CmdMsgRespBlock),
                         reg_req->resp_addr, reg_req->resp_rkey);
